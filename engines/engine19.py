...
from util.utils import Inverse_normlize_Precipitation,\
    get_sample_time,plt_discrete_indexs_img,calculate_rmse,calculate_mae,Inverse_Log_Precipitation
from util.utils import get_mean_std
...
def train_one_epoch(train_loader,
    ...
    tq = tqdm(train_total_step, desc=f'Training Epoch: {str(epoch)}', mininterval=0.3)
    wavelet_loss = 0
    loss_cls = 0
    for _ in range(train_total_step):#train_total_step
        optimizer.zero_grad()
        images, targets, geo_lsm, class_labels = train_loader.next()

        images = images[:,2:,:,:].to('cuda')
...
def val_one_epoch(val_loader,
    ...
    for _ in range(val_total_step):#val_total_step
        with torch.no_grad():

            lr, hr, geo_lsm, class_labels = val_loader.next()
            lr = lr[:,2:,:,:].to('cuda')
            hr = hr[:,0,:,:].unsqueeze(1)
            geo_lsm = geo_lsm.to('cuda')
            ...
            for j in range(hr.shape[0]):  # hr.shape[0]
                ...
                lr_img = lr[j,0,:,:].cpu().numpy()
                sr_img = sr[j,0,:,:].cpu().numpy()
                gt_img = hr[j,0,:,:].cpu().numpy()
                # Inverse Normlization
                """
                lr_img_inverse = Inverse_normlize_Precipitation(lr_img, lr_statistics_path)
                sr_img_inverse = Inverse_normlize_Precipitation(sr_img, lr_statistics_path)
                gt_img_inverse = Inverse_normlize_Precipitation(gt_img, hr_statistics_path)
                """

                # lr_img_inverse = Inverse_normlize_Precipitation(lr_img, 'lr')
                # sr_img_inverse = Inverse_normlize_Precipitation(sr_img, 'sr')
                # gt_img_inverse = Inverse_normlize_Precipitation(gt_img, 'gt')

                # lr_img_inverse = lr_img * lr_tp_std + lr_tp_mean
                # # sr_img_inverse = sr_img * hr_tp_std +hr_tp_mean
                # sr_img_inverse = sr_img
                # gt_img_inverse = gt_img

                lr_img_inverse = Inverse_Log_Precipitation(lr_img)
                sr_img_inverse = Inverse_Log_Precipitation(sr_img)
                gt_img_inverse = Inverse_Log_Precipitation(gt_img)
                ...
                if is_plt:
                    ...
                    plt_discrete_indexs_img(lr_img_inverse,gt_img_inverse,sr_img_inverse,img_outdir)
                ...
                rmse = calculate_rmse(sr_img_inverse,gt_img_inverse)
    ...
    log_info = f'[Validation] epoch: {epoch}, PSNR: {mean_psnr_scores:.4f}, ' \
               f'SSIM: {mean_ssim_scores:.4f},Accuracy: {mean_acc_scores:.4f},' \
