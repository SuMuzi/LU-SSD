...
class IntensityUpsampler(nn.Module):

    def __init__(self, rs_factor=8, base=64,out_channels=1):
        super().__init__()
        self.Up_Sample = Upsample(rs_factor, base)
        self.reg = nn.Conv2d(base, out_channels, 3, 1, 1)

    def forward(self, x):
        return self.reg(self.Up_Sample(x))
