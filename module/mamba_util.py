...
def bulid_act(act_name):
    ...
    act = None
    norm = None
    if act_name in active_map:
        act = active_map[act_name]
    else:
        ValueError(f'error activate function name {act_name} !!!')

    # if norm_name in norm_map:
    #     norm = norm_map[norm_name]
    # else:
    #     ValueError(f'error activate function name {norm_name} !!!')
...
class ConvFFN_SCSSD2(nn.Module):

    def __init__(self, channels, out_channels,expansion=2, drop=0.0,act=nn.ReLU):
        super().__init__()
        ...
        self.norm_act = act
        self.linear2 = nn.Sequential(nn.Conv2d(self.dim2, self.out_channels, 1, 1, 0),
                                     act)
...
class ConvFFN_SCSSD(nn.Module):
    ...
    def __init__(self, channels, out_channels,expansion=2, drop=0.0,act=nn.ReLU):
        ...
        self.drop1 = nn.Dropout(drop, inplace=True)
        self.norm_act = nn.ReLU()
        self.linear2 = nn.Conv2d(self.dim2, self.out_channels, 1, 1, 0)
