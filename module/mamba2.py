...
class Mamba2_1(nn.Module):
    def __init__(
        ...
        device=None,
        dtype=None,
        linear_attn_duality=True, #False
        **kwargs
    ):
    ...
    def forward(self, u, H, W, seq_idx=None):
        ...
        #2D Convolution
        # print('```````````````````````````xBC.shape:{} ````````````````````````````'.format(xBC.shape))
...
class Mamba2_2(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        #self.norm = RMSNormGated(self.d_inner, eps=1e-5, norm_before_gate=False)
        self.norm = nn.LayerNorm(self.d_inner) #####
        self.out_proj = nn.Linear(self.d_inner, self.out_channel, bias=bias)
